...
async _recalculateCart(cart, opts = {}) {
  ...
  async toggleAssurancePlus({ user, body, query }) {
    const customerId = user?._id || user?.id || null;
    const sessionId = query?.sessionId || body?.sessionId || null;
    ...
    // Get Assurance Plus details from database
    const AssurancePlus = require("../models/AssurancePlus");
    const assurancePlusData = await AssurancePlus.findOne({ isActive: true });

    if (!assurancePlusData) {
      logger.error("Assurance Plus not configured");
      throw new Error("Assurance Plus protection plan is not available");
    }

    // Update cart with Assurance Plus selection
    cart.assurancePlus = {
      isSelected: isSelected === true,
      price: isSelected ? assurancePlusData.price : 0,
      name: assurancePlusData.name,
      image: assurancePlusData.image,
    };

    await cart.save();

    logger.info({
      event: "ASSURANCE_PLUS_TOGGLED",
      customerId: customerId || "guest",
      sessionId,
      isSelected,
      price: cart.assurancePlus.price,
    });

    // Recalculate cart to populate products and get full cart data
    await this._recalculateCart(cart);
    
    // Return cart in the same format as getCart for consistency
    return cart;
  }

  async getAssurancePlusConfig() {
    const AssurancePlus = require("../models/AssurancePlus");
    let config = await AssurancePlus.findOne({ isActive: true });

    // If no config exists, create default one
    if (!config) {
      config = await AssurancePlus.create({
        name: "Assurance Plus",
        description: "Extended protection plan for your purchase",
        price: 20,
        image: "https://res.cloudinary.com/dcvi4yizg/image/upload/v1764850165/Group_1000004853_yaabi1.png",
        isActive: true,
        features: [
